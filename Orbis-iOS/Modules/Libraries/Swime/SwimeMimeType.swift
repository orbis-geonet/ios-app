...
public struct SwimeMimeType {
  ...
  /// Number of bytes required for `SwimeMimeType` to be able to check if the
  /// given bytes match with its mime type magic number specifications.
  fileprivate let bytesCount: Int

  /// A function to check if the bytes match the `SwimeMimeType` specifications.
  fileprivate let matches: ([UInt8], Swime) -> Bool

  ///  Check if the given bytes matches with `SwimeMimeType`
  ...
  /// List of all supported `SwimeMimeType`s
  public static let all: [SwimeMimeType] = [
    SwimeMimeType(
      mime: "image/jpeg",
      ext: "jpg",
      type: .jpg,
