...
class IGVideoCacheManager {
    ...
    func getFile(for stringUrl: String, completionHandler: @escaping (Response) -> Void) {
        ...
        if stringUrl.isNotUrlLink {
            let ref = stringUrl.getFirebaseFileStorageReference(storage: .postVideo)
            if let url = OrbisVideoCacheManager.shared.getUrl(forUrl: ref.fullPath) {
                downloadAndStoreContents(ofUrl: url, completionHandler: completionHandler)
                return
            }
            else {
                ref.downloadURL { [weak self] url, err in
                    if let videoUrl = url {
                        ...
                        self?.downloadAndStoreContents(ofUrl: videoUrl.absoluteString, completionHandler: completionHandler)
                        return
                    }
                    else {
                        completionHandler(Result.failure(VideoError.fileRetrieveError))
            ...
            downloadAndStoreContents(ofUrl: stringUrl, completionHandler: completionHandler)
        }
    }
    
    private func downloadAndStoreContents(ofUrl stringUrl: String, completionHandler: @escaping (Response) -> Void) {
        guard let file = directoryFor(stringUrl: stringUrl) else {
            completionHandler(Result.failure(VideoError.fileRetrieveError))
            return
        }
