//
//  CheckinItemCollectionViewCell.swift
//  Orbis-iOS
//
...
class CheckinItemCollectionViewCell: UICollectionViewCell {
    ...
    @IBOutlet weak var checkinPlaceIconView: UIImageView!
    ...
    @IBOutlet weak var checkinPlaceStackView: UIStackView!
    ...
    override func awakeFromNib() {
        ...
        let checkinPlaceIconTapGesture = UITapGestureRecognizer(target: self, action: #selector(didTapLocationStack(_:)))
        checkinPlaceIconView.isUserInteractionEnabled = true
        checkinPlaceIconView.addGestureRecognizer(checkinPlaceIconTapGesture)
        ...
        let checkinUserNameTapGesture = UITapGestureRecognizer(target: self, action: #selector(didTapCheckinUser(_:)))
        checkinUserNameLabel.isUserInteractionEnabled = true
        checkinUserNameLabel.addGestureRecognizer(checkinUserNameTapGesture)
        
        let checkinUserIconTapGesture = UITapGestureRecognizer(target: self, action: #selector(didTapCheckinUser(_:)))
        checkinUserProPicView.isUserInteractionEnabled = true
        checkinUserProPicView.addGestureRecognizer(checkinUserIconTapGesture)
        updateStaticText()
    ...
    private func updateCheckinPlaceProPic() {
        let placeholderImage = viewModel.checkinPlaceType.correspondingCircularImage(sizeRatio: 0.7, frameSize: 180.toCGFloat.relativeToIphone8Width())
        guard !viewModel.placeImageName.isEmpty else {
            checkinPlaceIconView.image = placeholderImage
            return
        }
        let proPicLink = viewModel.placeImageName
        checkinPlaceIconView.setSDWebImage(urlString: proPicLink, placeholderImage: placeholderImage, storageDirectory: .placePictures, sizeModifier: .fourHundred)
    }
}
