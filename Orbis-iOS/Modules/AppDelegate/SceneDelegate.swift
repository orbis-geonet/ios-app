...
class SceneDelegate: UIResponder, UIWindowSceneDelegate {
    ...
    func scene(_ scene: UIScene, openURLContexts URLContexts: Set<UIOpenURLContext>) {
        ...
        _ = appDel.application(UIApplication.shared, open: url, options: [:])
    ...
    func scene(_ scene: UIScene, continue userActivity: NSUserActivity) {
        ...
        guard let appDel = UIApplication.shared.delegate as? AppDelegate else { return }
        _ = appDel.application(UIApplication.shared, continue: userActivity) { _ in
        }
    }
    ...
    func scene(_ scene: UIScene, willConnectTo session: UISceneSession, options connectionOptions: UIScene.ConnectionOptions) {
        ...
        if let userActivity = connectionOptions.userActivities.first, let url = userActivity.webpageURL {
            DynamicLinks.dynamicLinks().handleUniversalLink(url) { dynamicLink, error in
                ...
                guard let appDel = UIApplication.shared.delegate as? AppDelegate else { return }
                appDel.handleDeeplink(link: dynamicLink)
            }
        }
